...
def timebin_range(start, end):
    ...
    return pd.date_range(
        timebin(start), timebin(end), freq=pd.DateOffset(hours=BIN_SIZE)
    )
...
def timebin_glob(pathname, timefilter=None):
    ...
    for file in files:
        ...
        timebin = datetime.datetime.fromisoformat(
...
def load(
    path, reader, device, prefix=None, extension="*.csv", start=None, end=None
):
    """
    ...
    files = timebin_glob(
        path + "/**/" + device + "/" + prefix + extension, timefilter
    )
...
def sessionreader(file):
    ...
    data = pd.read_csv(
        file, header=None, skiprows=1, names=["time", "id", "weight", "event"]
    )
...
def videoclip(path, device, start=None, end=None):
    ...
    framedata = load(
        path,
        lambda file: videoreader(file).assign(
            path=os.path.splitext(file)[0] + ".avi"
        ),
        device=device,
        extension="*.csv",
        start=start,
...
def pelletdata(path, device, start=None, end=None):
    ...
    """
    command = harpdata(
        path, device, register=35, names=["bitmask"], start=start, end=end
    )
    beambreak = harpdata(
        path, device, register=32, names=["bitmask"], start=start, end=end
    )
