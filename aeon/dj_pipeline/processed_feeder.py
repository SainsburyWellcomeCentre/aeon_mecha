...
def activate(*, create_schema: bool = True, create_tables: bool = True) -> bool:
    ...
    (``-> streams.FeederEncoder``) picks up any reload.
    ...
    # (`-> streams.FeederEncoder`) picks up any reload done by streams_maker
    # or the test fixture.
    ...
    try:
        ...
        logger.warning(f"processed_feeder not activated — streams import failed: {e}")
        return False
