...
def load(root, reader, start=None, end=None, time=None, tolerance=None):
    ...
    if start is not None or end is not None:
        ...
        if all(
            item[0][1]
            == datetime.datetime.fromisoformat(":".join(item[0][0].rsplit("-", 2)))
            for item in files
        ):
            # non-chunked data
            files = [item for item in files if start <= item[0][1] <= end]
        else:
            # chunked data
            files = [item for item in files if item[0][1] in chunkfilter]
    else:
        chunkfilter = None
    ...
    if chunkfilter is not None:
        try:
            ...
            if not data.index.has_duplicates:
                ...
                warnings.warn(
                    "data index for {0} contains duplicate keys!".format(reader.pattern)
                )
