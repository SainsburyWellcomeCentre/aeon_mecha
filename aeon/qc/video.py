...
for device in devicenames:
    ...
    for (key, period) in videochunks.groupby(by="epoch"):
        ...
        for chunk in period.itertuples():
            ...
            print(
                "[{1}] Analysing {0} {2}... ".format(device, key, chunk.Index), end=""
            )
            ...
            deltas["frame_offset"] = (
                deltas.hw_counter_delta - 1
            ).cumsum() + frame_offset
            drop_count = deltas.frame_offset.iloc[-1]
            max_harp_delta = deltas.time_delta.max().total_seconds()
            max_camera_delta = (
                deltas.hw_timestamp_delta.max() / 1e9
            )  # convert nanoseconds to seconds
            print(
                "drops: {0} frameOffset: {1}  maxHarpDelta: {2} s  maxCameraDelta: {3} s".format(
