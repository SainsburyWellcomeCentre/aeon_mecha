...
class TestPynappleCodecRoundTrip:
    ...
    def test_summary_is_queryable_without_decoding_the_object(self, mock_pynapple_table, mock_tsgroup):
        """Test that a caller can learn the type and size without fetching the object.

        The whole point of putting a summary in the JSON column: ``proj`` on a JSON
        path returns a plain scalar and never runs the codec, so no ``.npz`` is
        opened. Restriction works the same way, which is how a user filters a table
        holding several pynapple types down to the one they want.

        Inserting also exercises the MariaDB dict-to-JSON pymysql patch in
        ``aeon/dj_pipeline/__init__.py`` for a second JSON-dtype codec.
        """
        table, _schema, _loc = mock_pynapple_table
        table.insert1({"rec_id": 1, "data": mock_tsgroup})

        # projecting a JSON path gives a scalar column, not a decoded TsGroup
        projected = table.proj(kind='data->>"$.kind"', n_events='data->>"$.n_events"').fetch1()
        assert projected["kind"] == "TsGroup"
        assert int(projected["n_events"]) == sum(len(mock_tsgroup[u]) for u in mock_tsgroup.index)
        assert "data" not in projected  # nothing was decoded

        # and the same path restricts
        assert len(table & {"data.kind": "TsGroup"}) == 1
        assert len(table & {"data.kind": "TsdFrame"}) == 0
