...
def test_epoch_chunk_ingestion(test_params, pipeline, epoch_chunk_ingestion):
    ...
    epoch_count = len(acquisition.Epoch & {"experiment_name": test_params["experiment_name"]})
    chunk_count = len(acquisition.Chunk & {"experiment_name": test_params["experiment_name"]})
    if epoch_count != test_params["epoch_count"]:
        raise AssertionError(f"Expected {test_params['epoch_count']} epochs, but got {epoch_count}.")

    if chunk_count != test_params["chunk_count"]:
        raise AssertionError(f"Expected {test_params['chunk_count']} chunks, but got {chunk_count}.")
